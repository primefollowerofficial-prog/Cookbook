// =========================================================
// new.js — embedded page-flip engine (originally page-flip.browser.js)
// PLUS all custom book logic. No external CDN script required.
// =========================================================
...
// =========================================================
// Custom app logic layered on top of the embedded page-flip
// library above. Nothing below this line is part of the
// original library — it only drives the book UI.
// =========================================================

document.addEventListener('DOMContentLoaded', () => {

    // Safety check: stop here with a clear error if the library didn't load,
    // instead of silently failing.
    if (typeof St === 'undefined') {
        document.getElementById('book').innerHTML =
            '<p style="color:red;padding:20px;">page-flip library failed to load.</p>';
        console.error('St is undefined — the embedded page-flip library did not initialize.');
        return;
    }

    // ---------- 1. BUILD PAGE LIST (bookpage0.png ... bookpage113.png) ----------
    const firstPage = 0;
    const lastPage = 113;
    const pages = [];
    for (let i = firstPage; i <= lastPage; i++) {
        pages.push(`images/bookpage${i}.png`);
    }

    const bookEl = document.getElementById('book');

    // Build one <div class="my-page"><img></div> per page for HTML mode.
    // HTML mode keeps native <img> rendering -> crisp on zoom/retina,
    // instead of canvas mode which downsamples and blurs.
    pages.forEach((src, idx) => {
        ...
        // Only the actual front cover (bookpage0.png) gets the cardboard look.
        // Every other page — including the last one — reads as a normal paper page.
        if (idx === 0) {
    ...
    // ---------- 2. INIT PAGEFLIP (responsive / mobile friendly) ----------
    const pageFlip = new St.PageFlip(bookEl, {
        width: 500,
        height: 750,   // matches the real 2:3 page image ratio — no more white side bars
        size: 'stretch',
        minWidth: 220,
        maxWidth: 750, // capped so a 2-page desktop spread stays comfortably inside the viewport
        minHeight: 330,
        maxHeight: 1125,
        ...
        usePortrait: true,
        useMouseEvents: true,
        startPage: 0   // force it to always open at bookpage1
    });

    // Listen for init BEFORE calling loadFromHTML (required by the library)
    pageFlip.on('init', () => {
        console.log('✅ Book initialized. Page count:', pageFlip.getPageCount());
        updatePageNum();
    });
    ...
    function updatePageNum() {
        pageNumEl.textContent = `${pageFlip.getCurrentPageIndex() + 1} of ${pageFlip.getPageCount()}`;
    }
    updatePageNum();
    ...
    const bandColors = [
        ...
        'rgba(210, 255, 215, 0.55)', // very light green
        'rgba(205, 230, 255, 0.55)'  // very light blue
    ];
    // Same bands as above but a touch more opaque so the buttons in the
    // bottom bar stay easy to read against them.
    const controlBandColors = [
    ...
    // ---------- 5b. DOWNLOAD PDF BUTTON ----------
    const downloadBtn = document.getElementById('downloadBtn');
    if (downloadBtn) {
        downloadBtn.addEventListener('click', () => {
            const link = document.createElement('a');
            link.href = 'images/Ebook.pdf';
    ...
    let vStartDist = 0, vStartScale = 1, vStartPanX = 0, vStartPanY = 0, vMid = null;

    function applyViewerTransform() {
        viewerImg.style.transform = `translate(${vPanX}px, ${vPanY}px) scale(${vScale})`;
    }
    ...
    function openViewer(index) {
        viewerIndex = index;
        viewerImg.src = pages[viewerIndex];
        resetViewerZoom();
    ...
    function showViewerIndex(newIndex) {
    ...
    // Open fullscreen viewer on tapping/clicking a page image.
    // NOTE: the page-flip engine calls preventDefault() on touchstart (since
    // mobileScrollSupport is false), which suppresses the browser's normal
    // synthetic 'click' event on touch devices. So we detect taps manually
    // via touchstart/touchend, and keep 'click' as a fallback for mouse users.
    function findPageImg(target) {
        const img = target.closest && target.closest('.my-page img');
        return img || null;
    }

    function openViewerForImg(img) {
        const idx = pages.indexOf(img.getAttribute('src'));
        if (idx !== -1) openViewer(idx);
    }

    let tapStartX = 0, tapStartY = 0, tapStartTime = 0, tapTarget = null;
    bookEl.addEventListener('touchstart', (e) => {
        if (e.touches.length !== 1) { tapTarget = null; return; }
        const img = findPageImg(e.target);
        if (!img) { tapTarget = null; return; }
        tapTarget = img;
    ...
    bookEl.addEventListener('touchend', (e) => {
        ...
        // Only treat as a tap (not a drag/page-flip gesture) if movement is small and quick
        if (dx < 12 && dy < 12 && elapsed < 400) {
            openViewerForImg(tapTarget);
        }
        tapTarget = null;
    });
    ...
    bookEl.addEventListener('click', (e) => {
        const img = findPageImg(e.target);
        if (img) openViewerForImg(img);
    });

    viewerClose.addEventListener('click', closeViewer);
    viewerPrev.addEventListener('click', () => showViewerIndex(viewerIndex - 1));
    viewerNext.addEventListener('click', () => showViewerIndex(viewerIndex + 1));

    // Pinch-to-zoom, scoped only to the fullscreen viewer
    ...
    viewerImg.addEventListener('touchstart', (e) => {
        if (e.touches.length === 2) {
            vStartDist = getDist(e.touches);
            vStartScale = vScale;
            vMid = getMid(e.touches);
            vStartPanX = vPanX;
            vStartPanY = vPanY;
        }
    }, { passive: true });
    ...
    viewerImg.addEventListener('touchmove', (e) => {
        if (e.touches.length === 2) {
            ...
            let scale = (newDist / vStartDist) * vStartScale;
            scale = Math.min(Math.max(scale, 1), 5);
            vScale = scale;
            const mid = getMid(e.touches);
            vPanX = vStartPanX + (mid.x - vMid.x);
            vPanY = vStartPanY + (mid.y - vMid.y);
            applyViewerTransform();
        }
    }, { passive: false });

    // double-tap to reset zoom inside viewer
    let vLastTap = 0;
    viewerImg.addEventListener('touchend', (e) => {
        if (e.touches.length === 0) {
            const now = Date.now();
            if (now - vLastTap < 300) resetViewerZoom();
            vLastTap = now;
        }
    });

    // Single-finger drag to pan once zoomed in
    let vDragging = false, vDragStartX = 0, vDragStartY = 0, vDragPanStartX = 0, vDragPanStartY = 0;
    viewerImg.addEventListener('touchstart', (e) => {
        if (e.touches.length === 1 && vScale > 1) {
            vDragging = true;
            vDragStartX = e.touches[0].clientX;
            vDragStartY = e.touches[0].clientY;
            vDragPanStartX = vPanX;
            vDragPanStartY = vPanY;
        }
    }, { passive: true });

    viewerImg.addEventListener('touchmove', (e) => {
        if (vDragging && e.touches.length === 1) {
            e.preventDefault();
            vPanX = vDragPanStartX + (e.touches[0].clientX - vDragStartX);
            vPanY = vDragPanStartY + (e.touches[0].clientY - vDragStartY);
            applyViewerTransform();
        }
    }, { passive: false });

    viewerImg.addEventListener('touchend', () => { vDragging = false; });

    // Mouse wheel zoom (desktop) centered roughly on cursor
    viewer.addEventListener('wheel', (e) => {
        e.preventDefault();
        const delta = e.deltaY < 0 ? 0.15 : -0.15;
        vScale = Math.min(Math.max(vScale + delta, 1), 5);
        if (vScale === 1) { vPanX = 0; vPanY = 0; }
        applyViewerTransform();
    }, { passive: false });

    // Debug helper: tells you exactly what's wrong in the console
